...
class CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, label):
        ...
        zmax = torch.max(logits,dim=1)[0]
        
        zmax = torch.reshape(zmax, (logits.shape[0],1))
        z_max = zmax.repeat(1,logits.shape[1])
         # 1.2: calculate exps = exp(z - z_max)
        z_new = torch.zeros_like(logits)
        z_new = logits-z_max
        # 1.3: calculate p = softmax(y - y_max) 
        #q = torch.softmax(z_new, dim=1)   #自己实现softmax
        q = torch.zeros([logits.shape[0],logits.shape[1]])
        for i in range(logits.shape[0]):
            q[i,:] = torch.exp(z_new[i,:])/torch.sum(torch.exp(z_new[i,:]))
        ...
        #q=torch.softmax(logits,dim=1)         #这个函数要自己写，logits是网络输出，后面是在第二维计算
